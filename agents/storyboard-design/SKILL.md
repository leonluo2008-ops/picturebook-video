...
| **3.5-5s**（中句）| 1-4s | **中句档** | 5 | 1.5-1-朗读-2-1（建立 1.5s + 跃入 1s + 朗读 + 目标词重读 2s + 末帧 1s）| **7-10s** | Pic2 Clip 8 收势（~3.35s + 退场感）→ 8-10s |
...
- Clip 8（let's learn... ~3.35s + 收势）→ 中句档 **8-10s**
- **总时长** = 5+4+5+5+6+6+6+10 = **47s**（v0.7.1+pic7 96s → v1.0.0 旧档 53s → v1.0.0 修复档 48s → v1.0.1 进一步压 47s = 节省 51%）
...
- 标准词组末帧消化 **≥ 1s**（朗读完留 1s 让观众消化文字）
- 收势/退场末帧消化 **≥ 2s**（Cat 范式：韵词重读 + 画面停留给小朋友消化时间）
