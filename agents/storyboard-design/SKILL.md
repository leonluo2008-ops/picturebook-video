...
|---|---|---|---|---|---|---|
| **< 1.5s**（极短）| 1.5-3s | **极短档** | 3 | 1-朗读-1（建立 1s + 朗读 + 末帧 1s）| **3-4s** | Good afternoon / 1 词 |
| **1.5-3s**（短句）| 1.5-6s | **短句档** | 4 | 1-1-朗读-1（建立 1s + 跃入 1s + 朗读+消化 + 末帧 1s）| **4-6s** | Pic2 Clip 1-4（~2.3s）→ 4-5s |
| **3-5s**（中句）| 3-10s | **中句档** | 5 | 2-1-朗读-消化-收势（建立 2s + 跃入 1s + 朗读 + 目标词重读 + 收势）| **7-10s** | Pic2 Clip 5-7（~3.15s）→ 5-6s |
| **5-8s**（长句）| 5-16s | **长句档** | 5-6 | 2-1-3-消化-收势 | **10-15s** | Pic2 Clip 8 收势（~3.35s + 退场感）→ 10-11s |
| **> 8s** | — | 走 v15.1 拆 | — | — | — | Cat 6 Clip |

**Pic2 实测总时长**（B 输出 avg 2.75s/句）：
- Clip 1-4（旁白 ~2.3s）→ 短句档 4-5s
- Clip 5-7（旁白 ~3.15s）→ 短句档偏长 5-6s
- Clip 8（旁白 ~3.35s + 收势）→ 中句档 8-10s
- **总时长** = 4×5 + 3×6 + 10 = **48s**（v0.7.1+pic7 96s → v1.0.0 旧档 53s → v1.0.0 新档 48s = 节省 50%）

**关键反模式**：
- ❌ 短句用长档（"Good afternoon" 1.5s 套 6-7s = 凭空加 4-5s 空镜）
- ❌ 节奏公式 = 模板套数（应该是动作成本相加）
...
中句档 2-1-朗读-消化-收势  = 建立 2s + 跃入 1s + 朗读 + 目标词重读 + 收势  = 7-10s（Pic2 Clip 5-7 验证）
