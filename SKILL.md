...
| **Dog 绘本 v1.2.0+pic25 5 步调度 Dog 实战总结** ⭐ 2026-06-10 实战数据 | `references/2026-06-10-dog-v1.2.0+pic25-5步调度-Dog实战总结.md`（**v1.2.0+pic25 6 步调度 + 4 子 agent 架构的首本实战验证绘本** · Dog 8 段 v3 prompt 全部按新规范跑通 · 8/8 整数档 100% 命中 + md5 全唯一 · 3 并发 + 2 并发分批跑 = 596s ≈ 10 分钟（节省 60%）· 4 子 agent 实际调用 0/4 = 上下文隔离器价值）|
| **Dog 绘本 v1.2.0+pic25 多 agent 重构与 5 步调度拍板** ⭐ 2026-06-10 架构级重构 | `references/2026-06-10-dog-v1.2.0+pic25-多agent重构与5步调度拍板.md`（**用户两次纠错**让我意识到 picturebook-video 的多 agent 调度**根本没真用** = 5 件事全在主 agent 里跑 = 上下文污染 · 5 关键改动 = ① 删 A 风格识别子 agent（风格 = 主 agent native vision 视觉识别一部分）② 新增 BC 节奏/时长子 agent ③ 重写 C 子 agent = 纯分镜脚本（不拼 prompt）④ 新增 P 提示词编段子 agent ⑤ 5 步 → 6 步调度 · v1.2.0+pic25 Dog v3 实战验证 8/8 整数档 100% 命中 + 6 新铁律全验证 + 3 并发 + 2 并发分批跑 596s ≈ 10 分钟）|
| **达尔文 test-prompts 撞 100% = 幻觉天花板检测方法** ⭐ 2026-06-10 元教训 | `references/2026-06-10-darwin-100%-幻觉天花板检测方法.md`（**v1.2.0+pic22 撞 100% 撞成幻觉天花板教训** = 5/5 旧 test-prompts 引用已废 # 编号铁律 + 不存在的 references 文件 = 脚本匹配命中 = 虚假 100% · 真实覆盖率 0% · 3 步检测方法 = ① grep 已废 # 编号 ② Python 检查引用文件存在 ③ 全文搜真实覆盖率 · v1.2.0+pic22/23/25 三轮撞天花板战绩对比表）|
