...
## ⚠️ 启动前必问 + 必避（2026-06-05 Cat 绘本踩坑 · 关键教训 #32 #33 #34 #35 · 启动**最前**）

> **核心问题**：本次踩坑源于"按图片合并不按旁白 + 拖长韵算错 + 拍脑袋设上限 + 不查 CLI 就提交"。
> 修复方案：**启动任何绘本任务前先跑 4 必查**（铁律 32-35），不脑补。
>
> **修订（2026-06-05 Cat 启动）**：原 6 必问 → **3 必问 + 3 默认**。少问多做，**该默认的不问**。

### 🛑 末帧铁律 callout（v15.1 必读 · Cat 4a v3 实测）

> **铁律 #36**（启动绘本必看 · 不要做完才看到）：
> **末帧 = 最后一个"朗读+静默"镜头**，**不是 0.5s "收势"标签**。
>
> 用户原话（2026-06-05 Cat 4a v2→v3 反馈）：
> "把最后一个单词读完过后，仍然要像前面的每一个单词读完那样，留足够的呼吸、消化的时间，不要音频一读完，画面就结束了"
>
> **末帧镜头时长公式**：`朗读 1s + 静默 2-3s`（**跟前面单词镜头等长**，不缩水）
>
> **节奏公式（领读型标版 · Cat 4a v3 验证）**："2-1-3-3-3" = 短-短-长-长-长
> - 镜头一 2s（场景建立）+ 镜头二 1s（角色跃入）= **前 2 个动作镜头短**
> - 镜头三/四/五 各 3s（**单词镜头等长**）= **后单词镜头长且一致**
> - 末帧 = 镜头五（不追加"收势"镜头）
>
> **触发长旁白拆分**（铁律 #37）：`朗读总时长 + 单词数×2.5s 留白 > 15s` → 按"语义块"拆（3+3+1 不 7.5+7.5）
> 详见 `references/长旁白拆分规范-v15.1.md`。

### 3 必问 + 3 默认（启动前跟用户确认）

**必问**：

1. **切分方案**？（领读型 4-Clip 合并 / 叙事型 1图=1Clip / 自定义）
   - 领读型 → 按铁律 32 公式算旁白时长 → 决策树（铁律 33）
   - 叙事型 → 1图=1Clip
2. **范式风格**？（v15 绘本默认 / v14 备选 / v10 跨 Clip 同 BGM）
   - **绘本默认 = v15**（2026-06-05 起）
3. **特殊偏好**？（用户明确表达的非常规需求，如"只要音效不要 BGM"/"治愈系不要活泼"）

**默认**（不问，直接用 + 报告）：

| # | 项 | 默认值 | 依据 |
|---|----|--------|------|
| 1 | 视频比例 | **16:9** | 短视频平台标准（用户原话）|
| 2 | 目标时长 | **按铁律 32 公式算**（不擅自设上限）| 4-15s 都支持 |
| 3 | 运镜调性 | **活泼有动感**（领读型） / 治愈舒缓（情感型）| 铁律 27 |
| 4 | 运镜规则 | **按画面类型定制** | 铁律 27 6 类型表 |
| 5 | BGM 策略 | **A 静默氛围型**（默认） / B 调性匹配型（多情绪）| 双重禁 BGM 风格 |
| 6 | 切分模板 | 领读型 4-Clip 合并 `8+8+9+10=35s` | `leading-reading-4clip-pattern.md` |

### 5 必避（基于 5 类实测问题）

- ❌ 擅自合并 Clip（**按图片合并不按旁白**=反模式，铁律 33）
- ❌ 套硬编码默认值（4:3 / 8s / 4s）—— **必问用户**
- ❌ 私自延伸单 Clip 改动到全局（用户改 Clip 1 ≠ 全局 4s）
- ❌ 拍脑袋设时长上限（"12s 是最长""超 X 不行"——查文档不脑补，铁律 34）
- ❌ 默认重提交 task（先查 ark list 端点找 ID / 问用户）

### 4 必跑 · 启动前自检（铁律 32-35 · Cat 绘本实测补强）

```bash
# 1. 跑任何 seedance 命令前必跑（铁律 35）
python3 /home/luo/.hermes/profiles/huiben/skills/creative/seedance2.0-tool/seedance.py create --help

# 2. 算旁白朗读时长（铁律 32）
# 公式：中文 0.25s/字 · 英文 0.7s/词 · 拖长韵 1.0s/词
# 见 references/旁白朗读时长计算.md

# 3. 跑 3 步切分决策树（铁律 33）
# Step 1 算旁白 → Step 2 算 Clip 容量 → Step 3 决策树
...
- 避免误判：叫停 ≠ 立即停止，是"我**不再依赖这个结果**"
    ...
    - [ ] **铁律 31**：用户叫停 ≠ 立即停止，**任务继续跑完，结果废弃即可**？

26. **⚠️ 旁白优先 + 拖长韵 + 不擅自设上限 + 跑前必 --help（2026-06-05 Cat 猫绘本踩坑 · 关键教训 #32 #33 #34 #35 · 启动前必读）**：

    #### 铁律 32 · 旁白朗读时长公式（拖长韵 1.0s/词）

    **事件**（2026-06-05 Cat AT 家族）：用户原话"旁白'猫咪 cat 的 AT 家族，cat、hat、mat、bat、fat、rat 大集合！'，最长的句子，你竟然合并到 clip，还只有 9s"。
    ...
    **新公式**（替代旧"0.3s/字"统一公式）：
    ...
    | 1 中文字 | 0.25s |
    | 1 英文词 | 0.7s |
    | **拖长韵词（AT/IG/AN/EN 家族 6+ 词连读）** | **1.0s/词** ⚠️ |
    | 逗号/句号 | 0.3s |
    | 感叹号/问号 | 0.5s |

    **单 Clip 容量公式**（视频时长 - 0.5s 收势留白）：
    ...
    **旧公式陷阱**：`字数 × 0.3s/字` 统一套用 → **英文 12 词算 3.6s，实际 8.4s（低估 2.3 倍）**。

    **详细公式 + 决策树 + Cat/Say 对比**见 `references/旁白朗读时长计算.md`。

    #### 铁律 33 · 旁白优先于图片（合并 Clip 第一性原则）

    **事件**（2026-06-05 Cat 启动第一版切分）：用户原话"你得充分考虑旁白的朗读时长，不是按图片随意合并"。

    **核心规则**：
    1. **第一性原则 = 旁白朗读时长**（不是图片分组）
    2. Step 1 算每段旁白时长（铁律 32 公式）
    3. Step 2 算单 Clip 容量（视频时长 - 0.5s 收势）
    4. Step 3 按决策树切分：
       ```
       每段旁白 ≤ 4s 单段容量？
       ├── 是 → 1图=1Clip
       └── 否 → 合并后 ≤ 7.5s（8s 容量）？
                ├── 是 → 2图=1Clip（场景相似 + 弱叙事弧）
                └── 否 → 合并后 ≤ 11.5s（12s 容量）？
                         ├── 是 → 3图=1Clip
                         └── 否 → 4图=1Clip 12s Clip
       ```
    ...
    **反模式**：
    - ❌ "封面 1+2 / 物件 3+4+5 / 多人 6+7 / 收势 8"按图片分组（**不**算旁白）
    - ❌ "AT 家族是图 7 就跟图 6 合并"（图 6 旁白 4.3s + 图 7 旁白 10.25s = 14.55s 装 8s 装不下）
    - ✅ "图 7 旁白 10.25s > 8s 容量 → 独立 12s 段"

    #### 铁律 34 · 不擅自设时长上限

    **事件**（2026-06-05 Cat 提交前）：我自问"12s 是不是上限？"→ 拍脑袋"12s 是最长"→ 想"先验证能不能跑 12s"。

    **用户原话**（爆粗）："草，你TM哪来这么多问题？一直都说clip时长为：4-15s"

    **正确做法**：
    - ✅ seedance 2.0 单 Clip `4s ≤ duration ≤ 15s`——**12s 完全合规**
    - ✅ 拿不准时查文档（Phase 5 Step 2 表 + seedance2.0-tool SKILL）
    - ❌ 拍脑袋编"理论上限"
    - ❌ 自问"能不能跑 X" → 跑前用 X 试一下
    - ✅ 拿不准直接说"按 skill 默认值 X 跑"

    **与铁律 29 重复加固**：不擅自延伸用户指令 = 不擅自添加 skill 没写过的规则。

    #### 铁律 35 · 跑 seedance 前必先 `create --help`

    **事件**（2026-06-05 Cat Clip 4 提交过程）：4 次报错（`~` 路径错 / cat 素材位置错 / 子命令错 / 输出目录未建）。

    **根因**：**不查就拼参数提交**。我**默认假设** CLI 接参数直接跑（实际是 `create/status/wait` 子命令结构）。
    ...
    **看清楚 3 件事**：
    1. **子命令**（create / status / wait）—— 不是直接接参数
    ...
    **禁止**：
    ...
    - ❌ 假设 CLI 行为（"上次这样跑过"）
    - ❌ 用 `~` 写路径
    - ❌ 不 `mkdir -p output` 就拼 `--output <not_exist_dir>`

    #### 启动前 SOP 修订（6 必问 → 默认 3 必问 + 报 3 默认）

    > **修订原因**（2026-06-05 Cat 启动）：用户嫌"哪来这么多问题"——v15 启动时把 6 必问全列出来问，**该默认的没默认**。
    ...
    **6 必问 → 3 必问 + 3 默认**：
    ...
    | # | 项 | 处理 |
    |---|----|------|
    | 1 | 视频比例 | **默认 16:9**（绘本短视频标准）—— 不问 |
    | 2 | 目标时长 | **默认按 #32 公式算**（不擅自设上限）—— 不问 |
    | 3 | 切分方案 | **必问**（领读型 / 叙事型 / 自定义） |
    | 4 | 运镜调性 | **默认活泼有动感**（领读型）/ 治愈舒缓（情感型）—— 报告 |
    | 5 | 运镜规则 | **默认按画面类型定制**（铁律 27）—— 不问 |
    | 6 | 范式风格 | **默认 v15**（绘本默认 2026-06-05 起）—— 报告 |

    **判断口诀**：
    - "领读型" → 默认 v15 + 活泼有动感 + 4-Clip 合并模板（铁律 28 + 32）
    - "叙事型" → 默认 v15 + 1图=1Clip
    - "情感型/治愈型" → 默认 v15 + 治愈舒缓

    **报告 ≠ 询问**——我说"我打算 X"不是问你"OK 吗？"，但 X 应该是**有 skill 依据的 X**，不是脑补的 X。

    #### 4 必避 + 4 必查 · 自检（Cat 绘本启动前必跑）

    - [ ] **铁律 32**：每段旁白朗读时长算了吗（中文 0.25 / 英文 0.7 / **韵词 1.0**）？
    - [ ] **铁律 33**：切分方案是按旁白（不是按图片）？3 步决策树跑了吗？
    - [ ] **铁律 34**：时长在 4-15s 范围内？没拍脑袋设上限？
    - [ ] **铁律 35**：`seedance.py create --help` 查过了？路径用绝对路径？`mkdir -p output` 了？

    **配合 #25-#31 七铁律使用**——11 条铁律 = 绘本启动必读全套装。
---
...
| `references/2026-06-05-cat-pitfalls.md` | **2026-06-05 Cat 猫绘本踩坑对话日志**（旁白优先 / 拖长韵 / 不擅自设上限 / 跑前必 --help 4 个实测素材，对应 SKILL.md 关键教训 #32 #33 #34 #35） |
| `references/旁白朗读时长计算.md` | **旁白朗读时长计算公式 + 决策树**（中文 0.25s/字 · 英文 0.7s/词 · 拖长韵 1.0s/词 + 单 Clip 容量表 + Cat/Say 对比 + 4 个陷阱。对应 SKILL.md 关键教训 #32 #33） |
