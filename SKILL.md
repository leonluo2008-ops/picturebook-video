...
description: "绘本转儿童动画视频一站式调度 skill（**v1.1.0+pic20 · Rabbit 绘本分镜规范升级 · v15 导演思维版 = 唯一权威**）。把绘本简介 + N 张图 + 旁白 → **Step 0 文件用途澄清** → **Step 0.5 场景对位检查** → **Step 1 启动前 7 必问 + #1.5 数字约束数学验证** → 调 A 风格识别 + B 旁白量化（主 agent 干 · 并行）→ **Step 3.0 范式路由**（v7 2图=1Clip 走主 agent 直拼 / **v15 导演思维版** 6 段骨架 · 看 [`分镜设计规范-v15director.md`](./references/分镜设计规范-v15director.md)）→ 主 agent 写 6 段 prompt（**不调 C 拼老 v15 4 段/v6 5 段 · 已废弃**）→ **Step 4.0 seedance 范式二选一**（v15 走 `--ref-images` / v7 走 `--image`+`--last-frame`）→ D ≤2/批 + 续跑 → 发飞书 + 完整证据链（铁律 #76）。**v1.1.0 净化**：4 个老 references 彻底删除（`v6-5段骨架-模板.md` / `v15-4段骨架-模板.md` / `2026-06-07-pic4-no-v6-final.md` / `2026-06-07-pic4-no-v5-rhythm-formula.md`）· Rabbit newclip1 v2 实战验证新规范（5s · 3 镜头 · 1657 字符 · 5 分钟跑完 · md5 bdd229bd）。**触发词**：绘本视频、绘本转视频、绘本动画、绘本生成视频、picturebook video、绘本做视频。"
license: Apache-2-2
metadata:
  hermes:
...
> ## ⚠️ 2026-06-10 净化说明 · 分镜设计规范 v15 导演思维版为唯一权威
>
> **用户原话**（2026-06-10 落定净化决策）："**以后 clip 分镜规范以本轮设计实测的结构为准，移除其它版本，避免污染**"
>
> **本次净化动作**（4 个老 references 已彻底删除）：
> - ❌ `references/v6-5段骨架-模板.md` （v1.0.3+pic12 老 v6 模板）
> - ❌ `references/v15-4段骨架-模板.md` （v1.0.3+pic12 老 v15 模板）
> - ❌ `references/2026-06-07-pic4-no-v6-final.md` （Pic4 No v6 实战沉淀）
> - ❌ `references/2026-06-07-pic4-no-v5-rhythm-formula.md` （Pic4 v5 节奏公式 · 按秒数硬切翻车）
>
> **唯一权威分镜规范**：
> - ✅ `references/分镜设计规范-v15director.md`（v1.0.0 · 2026-06-10 Rabbit newclip1 v2 实战验证）
>   - 6 段骨架（素材角色声明 + 主体定义 + **多镜头时间线分镜** + 整段音频约束 + 画面禁令 + Storyboard 引导）
>   - 镜头数算法（5s=2-3 镜头 / 12s=4-5 镜头 / 14s=5-6 镜头）
>   - 每镜头 4 逻辑齐全（运镜 + 动作 + 位置 + 音频内联）
>   - 完整 Rabbit newclip1 v2 验证通过 prompt 模板（5s · 3 镜头 · 1657 字符 · 5 分钟跑完）
>
> **SKILL.md 内部 20+ 处老 v6/v15/v15.1 引用保留**（Pic4-Pic7 历史决策依据）—— **但**：
> - 任何"v15 4 段""v6 5 段""v15.1 长旁白""v6 整数时长"等老术语出现 = **必须同时看新规范**
> - 老范本**只作历史决策依据**（Pic4-7 翻车链沉淀）= **不**作新 prompt 写法参考
> - 写新 prompt 时 = **只看 `分镜设计规范-v15director.md`**
> - **不调 C 拼 v15/v6 模板**（v15 4 段/v6 5 段 = 旧范本，已被新规范 6 段骨架覆盖）
> - **不引用 v15 4 段/v6 5 段术语**——直接用新规范术语（"6 段骨架"/"镜头 1-N"/"4 逻辑"等）
>
> **本说明触发场景**：任何新绘本 prompt 编写 / 主 agent 拼 v6/v15 prompt 时 / 子 agent C 调度时 / 老 references 引用解析时
...
## 🔥 真铁律速查（12 条 · A 类红线 · 2026-06-10 精简版）

> **定义**：违反这 12 条 = 必然翻车 / 破坏产品 / 必然失败。
...
### 主 agent 行为铁律（元教训 · 5 条）

| # | 铁律（短句）| references 详细 |
|---|---|---|
| **#87** | **主 agent 跑完 = 发视频 = 不主动抽帧自检**（vision 是辅助不是真理） | Pic6 用户原话 |
| **⭐状态** | **主 agent 查 seedance task 状态走 execute_code + python urllib 直查 REST API**（不走 shell 管道 + seedance.py status） | Pic8 Rabbit qg6cg/r7vxs 误判卡死教训 |
...
| **#101** ⭐ 2026-06-10 Rabbit 净化 | **分镜设计规范 = v15 导演思维版为唯一权威（`分镜设计规范-v15director.md`）· 老 v6/v15/v15.1 模板全部废弃**（`v6-5段骨架-模板.md` / `v15-4段骨架-模板.md` / `2026-06-07-pic4-no-v6-final.md` / `2026-06-07-pic4-no-v5-rhythm-formula.md` **已删**）| 2026-06-10 用户原话"移除其它版本，避免污染" |

### 铁律精简原则（2026-06-10 用户原话 · 必读）
...
> - ⚠️ **用户说"删" = 真删 · 不擅自备份到 .deprecated 目录**（2026-06-10 Rabbit 净化踩坑）：用户原话"**移除其它版本，避免污染**"= **彻底删除**（`rm -f` 无备份）。❌ 反模式：擅自 `mv` 到 `废弃-YYYY-MM/` 目录加 `.deprecated` 后缀 = 混合"删"+"保" = 等于没删 = 还在污染。✅ 正解：用户说"删" → 直接 `rm -f` + 不留任何痕迹。**触发场景**：任何"清理/净化/移除"用户指令

### ⚠️ 反模式（2026-06-10 误读教训 · 必避）

> **本人本轮把 user 原话"铁律的**定义**要精简少"误读成"铁律**数量**要精简少"**——导致达尔文循环带偏见（每次评估倾向"砍"而不是"按 4 问筛选"）。**用户原话**（line 37）：「**铁律的定义是红线、底线，绝对不能逾越的原则。那么说它应该是精简少，而不是数量这么多。**」——**"精简少"形容的是铁律的"定义层面"**（红线本来就少，不是规则都叫铁律），**不是数量约束**。
> **正确做法**：每条新铁律 = 按 4 问筛选，**不问数量**。加新 = 4 通过 = 加；不通过 = 降级/删/并。**不要因"已经 12 条"而犹豫加新**。
> **本轮实证**：⭐状态铁律（execute_code+urllib 直查 REST API）4 问全过 → 加上后核心任务支持率 88%→100%，**加得对**。
...
Step 3 · 分镜设计 · **范式分流**：
   ...
   └─ **v15 导演思维版**（叙事/冒险/收势）→ **看 [`分镜设计规范-v15director.md`](./references/分镜设计规范-v15director.md) · 6 段骨架（**不调 C 拼老 v15 4 段** · C 老范本已废弃）**
   ↓
...
| **100**（v1.1.0+pic20 新增 · 2026-06-10 Pic8 Rabbit 实战 · 用户强烈纠错"v6 范本完全错"）| 看到 v6 "整段不分镜" / "5s 5 镜头" / "6 个 prompt 几乎完全一样" / 反馈"镜头呆板/单镜头/简单推镜头"时 | **v6 ≠ 减法版 = v15 4 段 + 文字持续可见段 · 多镜头时间线分镜 = 导演思维 · 镜头数必须匹配时长+旁白密度**。**Pic8 Rabbit 翻车链**：我误把 Pic8 Clip4 IT 家族 14s 特殊场景的 1440 字符"整段不分镜"版（Cat 范本精神"减所有额外加工"特例）当成通用 v6 范本 → 7 个新 Clip prompt = `@image1 + 风格 + 整段主题 + 音频约束 + 画面禁令`（**没有多镜头分镜**）→ 7 个视频 = 5 秒慢推 + 单镜头 + 文字一动不动 → 用户原话"**只有简单的推镜头，而且只有单镜头，这根本不能用！**" + "**V15 系列里面对镜头语言的设计就非常的好，你只要把 V15 里面关于镜头设计的东西，结合官方文档正确的写法，修正声音、音效，等一系列的东西**"。**3 个核心修复**（**改 v6 范式 = 加多镜头分镜段**）：① **v6 = v15 4 段 + 段 5 文字持续可见段**（**不是减法版**）② **多镜头时间线分镜 = 必填**（按事件不按秒数切，5s=2-3 镜头 / 8s=3-5 镜头 / 12s=4-5 镜头 / 14s=5-6 镜头）③ **每镜头 4 逻辑齐全**（运镜+动作+位置+音频内联，**不**只写段 4 兜底）。**3 个反模式**（**Pic8 实战翻车**）：❌ v6 "整段不分镜"= 1440 字符精简版 = 死板推镜头 = 通用绘本 100% 翻车（特例 14s IT 家族 OK）· ❌ 5s 装 5 镜头 = 每镜头 1s 画面糊 · ❌ 6 个 Clip prompt 几乎完全一样 = 6 个几乎一样的视频 = 失去绘本节奏。**修复路径**：`scripts/fill_v15_template.py` v15/v6 段 2 必填"镜头 1-N 描述 + 4 逻辑 + 拟声"，**主 agent 写 v6 prompt 必跑 6 步流程**（见 [references/v6-v15-导演思维+多镜头设计-2026-06-10.md](./references/v6-v15-导演思维+多镜头设计-2026-06-10.md) §6）。**判断口诀**：**"v6 = v15 + 文字持续可见段（不是减法版）· 多镜头分镜 = 必填（不是可选）· 镜头数 = 5s×2-3 / 8s×3-5 / 12s×4-5 / 14s×5-6"**。**详细实战数据**见 [references/v6-v15-导演思维+多镜头设计-2026-06-10.md](./references/v6-v15-导演思维+多镜头设计-2026-06-10.md)。|
...
| **分镜设计规范 · v15 导演思维版** ⭐ 2026-06-10 Rabbit 验证 v1.0.0 | `references/分镜设计规范-v15director.md`（**任何 seedance 2.0 绘本 prompt 写法的单一权威入口**· 6 段骨架 + 镜头数算法 + 4 逻辑齐全 + 运镜术语库 + 动作量化 + 情绪具象化 + Rabbit newclip1 v2 验证通过模板· **v6 旧版"整段不分镜"是翻车坑，新规范要求多镜头时间线分镜**）|
