...
- 维护陷阱 #24 sibling 写 references 易断链 = 跨 agent 协作时必跑自检 §3 + §3.1
...
- **#12 铁律** = 引用官方原话必带 4 维属性（原文+位置+语气+上下文）+ 漏 1 字 = 错全意
- **本节 #M2 §9** = 把 #12 铁律的"字面纪律"扩展到"规则表述纪律"= 写规则时也必须逐字用原话，不许改写
...
| 最终修 | "1 镜 1 运镜 + 多动作 OK（#12 铁律原文）" | "1 镜多动作 OK" | ✅ **逐字**引用 = 稳定 |

**3 步修 = 关键**：
- 第 1 步：找原文（grep "#12 铁律"）
- 第 2 步：逐字写（"1 镜多动作 OK"）
...
2. **单 clip 镜数 = 1 镜 1 种运镜 + 多动作 OK**（#12 铁律原文 · 不是 1 clip = 1 个"小短片"）
...
**⚠️ 判定 clip 节奏位 = 看段内容，不是看 clip 序号**（**2026-06-14 绘本实战沉淀**）：
...
| **末段时长凑整**（6s 凑 8s）| 真实 TTS 秒数 = 整数（4-15）= 不为了"显得饱满"凑整（再次印证 #18 铁律 · 2026-06-14 绘本整本验证沉淀）|
...
退出码 0 = 4/4 全过 · 1 = 任 1 项失败（详见上方 4 项检查表 + 下方维护陷阱 #14-#15 的 v2 bugfix 说明）。
