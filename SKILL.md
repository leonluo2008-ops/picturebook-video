...
description: "绘本转儿童动画视频一站式调度 skill（**v1.2.0+pic22 · 2026-06-10 净化：v7 范式彻底删除 + 升级运镜+音效+镜头名 3 个 Step 决定规则**）。v15 导演思维版 = **唯一**分镜规范（`references/分镜设计规范-v15director.md`）· v6/v15/v15.1 老范本全部废弃 · v7 范式（2图=1Clip + 首尾帧）彻底删除 · `--image`+`--last-frame` 首尾帧范式 = **禁用** · **默认'中景拉远定格 + 一声轻柔的微风声'模板 = 禁用**（Pic10 Hamster 5-8 段 6/6 段 100% 套用 = 翻车）· **镜头 1 命名'建立场景' = 禁用**（应用"动作+状态"命名 = 蹲伏姿态/侧面看门洞/跑轮开跑 等）。绘本简介 + N 张图 + 旁白 → Step 0 文件用途澄清 → Step 0.5 场景对位检查 → Step 1 启动前 7 必问 + #1.5 数字约束数学验证 → 调 A 风格识别 + B 旁白量化（主 agent 干 · 并行）→ 主 agent 写 6 段 prompt（看分镜设计规范-v15director.md §1 + §7.6 3 个 Step 决定规则）→ **3 并发 D 视频执行**（picturebook-workflow.md 铁律）→ 发飞书 + 完整证据链（铁律 #76）。**v1.2.0 净化**：15 个 v7 专有文件彻底删除（`templates/v7-prompt-template.md` / `cactus-clip1-v7.txt` / `cactus-clips-2-3-4-v7.txt` / `say-p1-v14-v7.txt` / `build-v7-clips-script.md` / `leading-reading-4clip-pattern.md` / `v7-vs-v15-paradigm-routing.md` / `clip-合并拆分决策-2026-06-10.md` / `铁律维护工作流-v7.1混合范式-2026-06-10.md` / `v7-12-check.md` / `2026-06-05-say-pitfalls.md` / `达尔文dry_run实践-v0.7.0.md` / `build_v8_clips.py` / `build_v9_clips.py` / `2026-06-10-rabbit-clip4-it-family.md`）。**触发词**：绘本视频、绘本转视频、绘本动画、绘本生成视频、picturebook video、绘本做视频。"
license: Apache-2-2
metadata:
  hermes:
...
## 🔥 决策时必看的新铁律速查（v1.2.0+pic21 · v7 已删版）
...
| **#99** | 看到任何"修复画面对齐"的 prompt 写法时 | **以参考图为基准 = 不约束 + 让 seedance 看图 = 任何镜头设计都 OK** |
| **#100** | 看到 v6 "整段不分镜" / 反馈"镜头呆板/单镜头/简单推镜头"时 | **v6 = v15 4 段 + 文字持续可见段 · 多镜头时间线分镜 = 必填 · 镜头数必须匹配时长+旁白密度** |
