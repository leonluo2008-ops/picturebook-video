...
description: "绘本转儿童动画视频标准流程（v5.0.18 图-段对位禁止凭 vision 文字相似性造异常 · 2026-07-23）。**单仓含 seedance_mcp/ 集成（uguu + Ark API + MCP 协议壳）** = 克隆 1 仓 = 完整可用，不依赖 seedance2.0-tool 仓。v8.1 动作模板（按事件分镜 · 替代 v8 运镜模板 · Potato BUG 修复） + 7 步流程 + 8 硬约束（v5.0.17 加 #7 = 图-段对位核对；含硬约束 #1 = 永远无 BGM / 硬约束 #6 = 镜头 4 件套）+ 9 案例 + SRT 驱动工作流 + 音频路由（路径 A 有 SRT = 旁白+音效 / 路径 B 无 SRT = 纯音效 / 路径 C/D spike/静默）。Step 1 接收需求 + SRT → Step 2 vision 自检（5 项必查 + 第 6 项图-段对位表 + 跟用户确认）→ Step 3 解析 SRT 真实时间戳 + 反推速率 → Step 4 旁白优先逐段合并 → Step 5 v8.1 动作模板 → Step 6 seedance 提交 → Step 7 端到端验证。"
license: Apache-2-2
metadata:
  hermes:
