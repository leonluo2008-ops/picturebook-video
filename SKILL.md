...
## ⚠️ 启动前必问 5 件事 + 必避 5 条（2026-06-05 Say 说绘本踩坑 · 关键教训 #31 · 启动**最前**）

> **核心问题**：本次踩坑都源于"默认值猜错 + 切分方案擅自定 + 私自延伸单 Clip 改动"。
> 修复方案：**启动任何绘本任务前必问 5 件事**，不脑补。
...
4. **范式风格**？（领读型同氛围 v10 / 调性匹配 v8 / 静默治愈 v7 / 强动感 v9）
...
5. **运镜规则**？（按画面类型定制 / 套公式 / 强动感开场）
   - 铁律 27 禁止套公式——按封面/动作/多人/情感/礼物/动态 6 种选
...
24. **⚠️ 旁白语言版本 + 彩色文字 + 运镜定制铁律（2026-06-05 Say 说绘本踩坑 · 关键教训 #25 #26 #27 · 启动前必读）**：
    ...
    **写 prompt 时的决策**：
    ...
    #### 铁律 27 扩展 · 运镜三禁（2026-06-05 Say 绘本补强 · 提示词设计核心）

    > **新增违规模式**：之前理解的"运镜禁公式"只盯住"中景→中近景缓推 3 秒"，但 Say 绘本第二轮 v3 prompt 写"镜头一/二/三/四/五"5 段结构 + 描述里有"0.5 秒后"/"3 秒后"等时间感词——**也算套公式**。
    > 根因：禁的是"运镜模板"和"时间公式"两层，**只挡一层 = 漏一层**。

    **运镜三禁**（写 prompt 时**必避**）：

    - ❌ **禁时间公式 1**：`0-1.5s / 1.5-3s` 时间段标注
    - ❌ **禁时间公式 2**：`0.5 秒后 / 3 秒后 / 停顿 2 秒` 时间感描述
    - ❌ **禁结构公式**：超过 4 镜头的"标准结构"（5 镜头起承转合也算）

    **运镜三写**（写 prompt 时**必写**）：

    - ✅ **只写"镜头一/二/三"** 序列（≤ 4 镜头）+ **无时间数字**
    - ✅ **只写"快速 / 极缓 / 几乎不动"** 节奏词
    - ✅ **只写"砰 / 叮 / 沙沙"** 拟声词

    **自检清单**（写完 prompt 必跑）：
    ```bash
    # 1. 数字 0-9 出现在 prompt 里 ≤ 3 次（仅允许时长 4-15s、720P 等参数）
    grep -oE "[0-9]+" <prompt> | wc -l
    # 2. "镜头" 出现次数 ≤ 4
    grep -oE "镜头[一二三四五]" <prompt> | wc -l
    # 3. "秒" 出现次数 ≤ 0（除参数外）
    grep -oE "[0-9.]+ *秒" <prompt> | wc -l
    ```
    ...
    **实测错误示例 vs 正确示例**：
    ...
    镜头二：双手快速举到脸颊两侧（0.5 秒后）；
    镜头三：镜头快速推进到脸部特写，文字"说Say!"同步弹出（1.5 秒后）；
    镜头四：镜头缓推持续，文字在画面中自然呼吸；
    镜头五（收势）：画面最终定格...

    ✅ 正版（3 镜头 + 无时间词）：
    镜头一：小熊从画面下方外快速跳入到中央；
    镜头二：双手快速举到脸颊两侧，嘴巴同步张大，文字"砰"弹出；
    镜头三（收势）：画面定格在小熊双手捂嘴 + 文字饱满呈现的姿态，镜头完全静止不动，停留极短时间后视频结束（约 0.5 秒——这条例外，收势时长必须明确）。
    ```
