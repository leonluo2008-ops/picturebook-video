...
（起/承：快速交代，约1秒）
镜头缓慢推进至面部特写，火光映照脸庞。（转：约2秒）
最终定格保持3秒不动，火焰稳定燃烧。（收势：3秒）
...
**情感镜头原则**：前段让路给收势，收势不是「动作进行中截断」，是「镜头在完成状态上停住」。
...
7. **收势阶段编写规范**：显式标注「收势阶段」+时长、写出收尾动作质感（缓慢拉远/定格/淡出）、写出停留时长（保持3秒不动）
8. **起/承阶段不能拖**：情感镜头的起/承阶段压缩到1-2秒，避免铺陈过多把时间吃光，导致收势没空间——Clip 9就是这个问题（2026-05-26实测）
9. **旁白处理**：Seedance 模型具备旁白生成能力，当前问题是还未找到正确写法，标注为「待后续完善」，不是「禁止写旁白」
9. **下载文件路径不固定**：seedance.py download 到 `--download` 指定目录，但文件名需用 task_id 查状态确认实际下载路径
