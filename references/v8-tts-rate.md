...
### 领读型绘本专用测算（**2026-06-17 多图蒙太奇绘本旁白实测沉淀 · v5.0.8 新增 · 铁律 #35**）

**领读型判定**：旁白格式 = "独立英文列 + 中文列含嵌入英文目标词"（如 "孔雀 PEACOCK!" / "孔雀是蓝绿色的，the peacock is blue and green."）

**TTS 实测行为**：TTS **只朗读中文旁白列**（已含英文目标词 = 读一遍即可）。**独立的英文旁白列 = 展示用，不参与 TTS 朗读**。

**AI 测算规则**（**仅供 Step 4 划分 Clip 时算拟时长 · 不改 TTS 引擎**）：

1. **只算中文列**（独立英文列 = 展示用 = 不算时长）
2. 嵌入英文在中文列里**合并念一遍**（不重复算）
3. 嵌入英文按 **英文 1.4 词/秒**算（跟档 3 速率一致）
4. **实测匹配目标**：AI 算的总时长尽量跟用户给 TTS 数值匹配（**差 ≤ 5s**）

**示例**（多图蒙太奇绘本 8 段）：
- 独立英文列 8 句（不参与朗读）
- 中文列 8 句（含嵌入英文）
- 只算中文列字数 + 嵌入英文按 1.4 词/秒 → 39.17s ≈ 用户给 TTS 39s ✅

**反模式（必避）**：
- ❌ 把独立英文列当朗读列算时长（重复计入 = +6~11s）
- ❌ 嵌入英文按中文速率算字符数（混淆语种）
- ❌ AI 算的时长跟用户 TTS 差 > 5s 不调整
