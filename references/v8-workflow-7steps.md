...
### Step 6.0 · 必走 · 单测（spike clip + 用户目检确认）· 2026-06-17 云服 8 并行翻车沉淀

**3 步必走**（主 agent 路径强约束 · 违反 = 红线违规）：

#### ① 跑 1 条 spike clip（最低费用）
...
ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 clip1-spike.mp4

# 发飞书给用户看效果（必带 MEDIA: 附件）
send_message action=send target=feishu message="[绘本名] Spike 单测完成，请目检确认。MEDIA:/path/clip1-spike.mp4 时长=Xs md5=XXX"

# ⚠️ 必等用户回 "OK / 通过 / 可以批量" 类确认 = 不许自判 / 不许沉默默认
```

#### ③ 才进入批量提交（Step 6.2 每轮 ≤ 3 并发）

**只有收到用户确认后**才进 Step 6.1 批量提交。

**反模式（必避）**：
- ❌ 8 个 Clip 一次性并行提交（云服多图 Clip 翻车原文）
- ❌ spike 跑完自判"应该 OK"不确认（自判 = 翻车）
- ❌ 用户没回复就当默认通过（沉默 ≠ 确认）
- ❌ spike clip 用 720p + 最长时长（费用浪费）
