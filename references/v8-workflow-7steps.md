...
### 领读型绘本专用规则（**2026-06-17 多图蒙太奇绘本旁白实测沉淀 · v5.0.8 新增 · 铁律 #35**）

**判定**：旁白格式 = "独立英文列 + 中文列含嵌入英文目标词" = 领读型绘本（如 "孔雀 PEACOCK!" / "孔雀是蓝绿色的，the peacock is blue and green."）。

**TTS 实测行为**：TTS 实际**只朗读中文旁白列**（已含英文目标词 = 读一遍即可）。**独立的英文旁白列 = 展示用，不参与 TTS 朗读**。

**AI 测算规则**（**仅供 Step 4 划分 Clip 时算拟时长 · 不改 TTS 引擎**）：

1. **只算中文列**（独立英文列 = 展示用 = 不算时长）
2. 嵌入英文在中文列里**合并念一遍**（不重复算）
3. 嵌入英文按 **英文 1.4 词/秒**算（跟铁律 #25 速率档一致）
4. **实测匹配目标**：AI 算的总时长尽量跟用户给 TTS 数值匹配（**差 ≤ 5s**，超出必调整拆分策略）

**判定口诀**："**领读型 = 双列 / 只算中文列 / 嵌入英文按 1.4 词/秒 / 实测匹配用户 TTS ≤ 5s**"

**反模式（必避）**：
- ❌ 把独立英文列当朗读列算时长（**重复计入** = 总时长 +6~11s）
- ❌ 嵌入英文按中文速率算字符数（混淆语种 = 失真）
- ❌ AI 算的时长跟用户 TTS 差 > 5s 不调整（实测匹配是目标，不是参考）
...
## Step 4 · Clip 划分（核心逻辑 = TTS 旁白时长能装下 + seedance 可分时序切换参考图）

### 合并决策（核心约束 · 2026-06-17 v5.0.7 修订 · 多图蒙太奇翻车沉淀）

**核心约束**：seedance 物理上限 = 单 Clip ≤ 15s · **拟时长 ∈ [4, 15]** 是合并/拆分的**唯一核心约束**。

**合并/拆分判断逻辑**：

1. **拟时长 = 合并 Clip 内所有旁白段 TTS 总时长 + 0.5s 末帧微动空间**
2. **拟时长 ≤ 15s ✅ = 允许合并任意参考图**（seedance 可按时序切换参考图 = 视觉不是障碍）
3. **拟时长 > 15s ❌ = 必拆 Clip**（拆依据 = 旁白切分点，不是视觉）
4. **拟时长 < 4s ❌ = 拆太碎 = 必合并**（增加合并的旁白段数）

### 多图 Clip 分镜时序写法（2 种范本都接受 · 多图蒙太奇翻车沉淀）

**范本 A · 英文官方基础版**（Seedance 官方文档推荐）：
```
from 0.0s to 5.0s @Image1 is the first shot, [房子运镜 + 动作 + 情绪 + 方向锚点];
from 5.0s to 10.0s @Image2 is the second shot, [鸟窝运镜 + 动作 + 情绪 + 方向锚点];
```

**范本 B · 中文镜头 N 分段版**（中文版可读性更高）：
```
镜头 1（0-5s）：参考 @Image1，[房子运镜 + 动作 + 情绪 + 方向锚点];
镜头 2（5-10s）：参考 @Image2，[鸟窝运镜 + 动作 + 情绪 + 方向锚点];
```

**核心机制（澄清 v5.0.7 之前我误解的点）**：
- 合并 Clip 内 = **按时序分镜切换参考图**（前 5s @Image1 → 后 5s @Image2）
- **不是** 1 个画面里 2 个主体同时出现
- **任何参考图都允许合并**（不相关图也能按时序切换 = seedance 模型能力覆盖）
- 用户原话："**视频它是有时长的**...10 秒的合并 clip 时长，前 5 秒是图一的参考图生成的视频，后 5 秒是图二的参考图生成的视频"

### 旁白节奏（拆点参考 · 非必要条件）

- 长句（> 5s 单句旁白）= 必拆独立 Clip（容纳阅读节奏）
- 短句连读（如 "HOME! 家" 仅 0.96s）= 可合并相邻 Clip（避免短句独占浪费时长）

### 反模式

- ❌ 拟时长 > 15s（旁白装不下 = 必拆）
- ❌ 拟时长 < 4s（拆太碎 = 单 Clip 时长不足 = 镜头没空间展开）
- ❌ 1 个画面里塞多个主体（违反分时序切换 = 必拆独立 Clip）
...
|---|---|
| 1 镜 1 图 | v8 默认（最安全） |
| 1 镜 N 图（N≤3）| 多图分时序切换 1 Clip（按拟时长 ≤ 15s 判断） |
| N 镜 N 图 | 故事弧需要（v8 推这种） |
| **任意 N Clip**（**v5.0.7 修订**）| **只要总时长 ≈ TTS + 每 Clip 拟时长 ∈ [4, 15] = 都合法** |
...
| generate_audio | **true**（绘本有声场景 · 自动加音效 + 旁白，不加 BGM）| v5.0.6 起绘本场景默认开音频；BGM 由 prompt 末尾约束"无背景音乐"排除（详铁律 #17 + #36）|
