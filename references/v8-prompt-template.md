...
### 单镜头时长倾向（v5.0.10 新增 · 官方教程示例 2-4s 节奏沉淀）
...
**v5.0.10 倾向区间**（**主 agent 路径 · 跨场景适用 · 不硬卡秒数**）：
- **单镜头时长倾向 2-4s**（参考官方 8s/4 镜头示例的节奏）
- **总时长 ≥ 4s 时 = 优先多镜头叙事**（用 2-4s 拆成多个镜头 = 每段独立运镜 = 比单镜头长运镜更稳）
- **总时长 ≤ 4s 时 = 单镜头**（seedance 物理下限 = 硬约束）
- **不硬卡秒数**（prompt 里不写"前 3 秒做 X"）= 违反官方"强行限制时长可能异常"警告
- **靠镜头切换 = 模型自然分时序**

**判定口诀**："**2-4s 镜头节奏 + 不卡秒数 + 多镜头叙事 = 避免机械**"

**反模式（必避）**：
- ❌ 单镜头时长 > 5s + 仅 1 个运镜（容易出现循环/丢失运镜/动作卡顿）
- ❌ prompt 硬卡"前 3 秒做 X / 4-6 秒做 Y"（违反官方"不精确时间"警告）
- ❌ 总时长 8s 仍用单镜头（应该拆 4 个 2s 镜头 = 匹配官方示例）

**示例**（绘本 8s 旁白）：
- ❌ 单镜头 8s + 1 个运镜 = 机械/卡顿
- ✅ 4 镜头 × 2s = 镜头 1 出现 / 镜头 2 转折 / 镜头 3 高潮 / 镜头 4 收尾（**不卡秒数**）

**跨 skill 适用**：所有用 seedance 的场景（绘本 / 漫剧 / AI 短剧 / 多图参考视频生成）
...
# 检查 11: 单镜头时长倾向（v5.0.10 新增 · 单镜头超 5s 易机械沉淀 · 详铁律 #37）
# 期望：单 Clip 时长 ≥ 4s 时 = 优先多镜头叙事（拆成 2-4s/镜头）= 避免单镜头 > 5s 机械/循环
# 失败：❌ 单 Clip 时长 > 5s + 0 处"镜头 N"标识 = 必改多镜头叙事
# 触发场景：绘本总时长 8s 仍用单镜头 + 1 个运镜 = 机械/卡顿
for clip in clip*-prompt.txt; do
  CLIP_DURATION=$(grep -E "^duration:" "$clip" 2>/dev/null | head -1 | awk '{print $2}')
  if [ -n "$CLIP_DURATION" ] && (( $(echo "$CLIP_DURATION > 5" | bc -l 2>/dev/null) )); then
    LENS_COUNT=$(grep -cE "镜头 [0-9]+|from [0-9.]+s to [0-9.]+s" "$clip" 2>/dev/null || echo "0")
    if [ "$LENS_COUNT" -le 1 ]; then
