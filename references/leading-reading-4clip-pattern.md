...
|| **跨场景双图合并不写"水平切到 X 场景"**（2026-06-06 Good afternoon 沉淀）| 模型不切场景，浪费双参考帧 | 每个镜头显式写"水平切到 X 场景 / 切回 Y 场景" |
|| **跨场景双图合并不写"负面隔离"段**（2026-06-06 沉淀）| 模型跨图召唤其他元素（鸭子出现在窗前、狐狸出现在树下等）| 加 "图片中的 X、Y、Z 元素不出现" 显式排除 |
|| **跨场景合并 3+ 张图**（2026-06-06 沉淀）| 模型会乱，元素乱入 | **最多 2 张图跨场景合并** |
