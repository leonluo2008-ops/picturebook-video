...
### 新增（Step 1.5 竖版参考图工作流 · 农民批实测）
- **新增 Step 1.5「竖版参考图工作流」**：横版素材 + 竖版 9:16 输出时必走。根因 = seedance 对比例不一致参考图居中真裁剪（1920×1200 喂 9:16 只剩中间 ~675px 竖带，带外元素模型看不见；官方「图片裁剪规则」实锤）。
- **三方案实验**（`references/vertical-reference-workflow-v5.0.20.md`）：A 提示词层强指令 = 带外元素仍消失（证伪）；B 预合成 9:16 画布 = 用户否决「和横版没区别」；✅ C 源头竖版 = 反推提示词 → 用户即梦批量出真 9:16 图 → 直喂，农民批 8/8 clip 一次过零返工。
- **反推双核对**（踩坑沉淀）：① 图↔旁白一致性（原始素材 2.jpg/4.jpg 本就重复场景配不同旁白，反推照抄放大雷）② 场景唯一性（重复页重设计，如「农民来了」→ 扛锄沿土路走来挥手）。
- **竖版提示词文件规范**：L4 四字段 + 全局外观锚点 + 文字控制警告，字符硬上限 2000（用户要求，实测 1491）。
- 首帧集成瞬态记录：frame0 轻微放大致边缘元素裁切仅单帧 ~42ms，0.4s 起恢复，无需返工（v1/v2 同现象，非推近措辞导致）。
- SKILL.md：版本行 v5.0.20 + Step 1.5 章节 + 硬约束表 #8。
